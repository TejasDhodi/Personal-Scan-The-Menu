...
// To Update the status of Processsing to Pending
...
const setOrderAsPaid = async (req, res) => {
    try {
        const setStatusTrue = await paymentModel.findByIdAndUpdate(req.params.id,
            {
                delivered: false,
...
// To Apply Pagination On Occupied Table
...
//         const orders = await paymentModel.find({ isTableOccupied: true });
//         console.log('Total occupied orders:', orders.length); // Log the length of the orders array
...
module.exports = {
    setProcessingStatusTrue, setDeliveryStatusTrue, setDeliveryStatusFalse, setOrderAsPaid,
    setPaginationOnProcessing, setPaginationOnPending, setPaginationOnDelivered,
